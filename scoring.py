...
def main():
  ...
  with open(embeddings_file, 'r') as f:
    ...
    features_matrix = np.zeros(shape=(N, dim), dtype=np.int)
    # Read embeddings
    for line in f.readlines():
      tokens = line.strip().split()
      features_matrix[int(tokens[0]), :] = [int(value) for value in tokens[1:]]
  '''
  features_matrix = np.load(embeddings_file, allow_pickle=True)
    '''
  ...
  ####################
  #node_labels = mat['node_labeled']
  ##print(node_labels)
  #features_matrix = features_matrix[node_labels.flatten(), :]
  #print(features_matrix.shape)
  ###################
